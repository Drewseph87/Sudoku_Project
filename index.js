...
const getRow = (puzzle, rowIndex) => {
  //Row Function to go through puzzle and find "searchvalue=rowIndex" and bring back what's in rowIndex
...
const getColumn = (puzzle, columnIndex) => {
  // Column Function to go through puzzle and find "searchvalue=columnIndex" and bring back
  let columnArr = []; // need to assign new array to grab numbers since column isn't array

  for (i = 0; i < puzzle.length; i++) {
    // looping to go through total puzzle.length for each row to start.
...
const getSection = (puzzle, x, y) => {
  ...
  for (let i = x; i < x + 3; i++) {
    // looping through 3 numbers due to +3 for x = rows = starting points of subgrid
    for (let j = y; j < y + 3; j++) {
      //looping through 3 numbers due to +3 for y = columns = starting points of subgrid
...
const includes1to9 = (numArray) => {
  for (let i = 1; i <= 9; i++) {
    //loop through a new array for 1-9
    let found = false; // assign a variable to false
    for (let j = 0; j <= numArray.length; j++) {
      //loop through values in array start wth first index.
      let value = numArray[j]; // assign the value of the index's
      if (value === i) {
        // if value found is same as I then i should be considered true
        found = true;
        break;
      }
    }
    if (!found) {
      // if value was not found true / value===i, then stop all process and return false
      return false;
    }
  }
  return true;
